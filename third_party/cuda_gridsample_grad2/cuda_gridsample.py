...
gridsample_grad2 = load(name='gridsample_grad2', sources=source_files, verbose=True)
...
class _GridSample2dBackward(torch.autograd.Function):
    ...
    def forward(ctx, grad_output, input, grid, padding_mode=0, align_corners=True):
        op, _ = torch._C._jit_get_operation('aten::grid_sampler_2d_backward') # Yulun: fix for pytorch 1.13
...
class _GridSample3dBackward(torch.autograd.Function):
    @staticmethod
    def forward(ctx, grad_output, input, grid, padding_mode=0, align_corners=True):
        op, _ = torch._C._jit_get_operation('aten::grid_sampler_3d_backward')  # Yulun: fix for pytorch 1.13
        if _use_pytorch_1_11_api:
            output_mask = (ctx.needs_input_grad[1], ctx.needs_input_grad[2])
            grad_input, grad_grid = op(grad_output, input, grid, 0, padding_mode, align_corners, output_mask)
        else:
